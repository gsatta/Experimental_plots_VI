...
index_vegetations <- c("BLUE", "GREEN","RED", "NIR", "DVI", "EVI", "GCI", "GDVI", 
                 "GEMI", "GNDVI", "GOSAVI", "GRVI", "GSAVI", "IPVI", "MSAVI2",
                 "MSR", "NDVI", "NDWI", "NLI", "OSAVI", "RDVI", "SAVI", "SR",
                 "VARI")
...
vi_directory <- "./ELABORATION/RASTER/VI"

# Nuova directory per i raster multilayer
output_directory <- "./ELABORATION/RASTER/VI_MERGED"
...
```{r}
VI_directory <- "./ELABORATION/RASTER/VI_MERGED"
```
...
raster_folder  <- "./ELABORATION/RASTER/VI_MERGED"
output_folder <- "./ELABORATION/RASTER/VI_EXTRACTED"
...
INPUT_folder <- "./ELABORATION/RASTER/VI_EXTRACTED"
...
# Save the list
saveRDS(shp_list, "./ELABORATION/LIST/shp.rds")

# Load the list
shp_list2 <- readRDS("./ELABORATION/LIST/shp.rds")
...
# Save the list
saveRDS(shp_list2, "./ELABORATION/LIST/shp2.rds" )
...
```{r}
shp_list3 <- readRDS("./ELABORATION/LIST/shp2.rds")
...
saveRDS(long_format_list, "./ELABORATION/LIST/long_format.rds" )
...
```{r}
long_format_list <- readRDS("./ELABORATION/LIST/long_format.rds")
...
# Save the long format list
saveRDS(long_format_list_2, "./ELABORATION/LIST/long_format2.rds")
```
...
```{r}
# long_format_list_2 <- readRDS("./ELABORATION/LIST/long_format2.rds")
# 
...
# 
# write.csv(vi_aggregated, "./ELABORATION/CSV/vi_aggregated.csv" )
```
...
# Impostazione del numero di core desiderato
num_cores <- 4  # Modifica a seconda del tuo hardware

# Caricamento della lista sf e conversione in data frame in parallelo
long_format_df <- mclapply(
  readRDS("./ELABORATION/LIST/long_format2.rds"),
  bind_rows,
  mc.cores = num_cores
) %>%
  bind_rows()

# Conversione del data frame in un data.table
long_format_dt <- as.data.table(long_format_df)

# Calcolo della media del VI per ogni COD, data, e VI in parallelo
vi_aggregated <- long_format_dt[, .(VALUE = mean(VALUE)), by = .(COD, date, VI)]

# O alternativamente, puoi usare la funzione na.omit()
...
vi_aggregated <- read_csv("./ELABORATION/CSV/vi_aggregated.csv")
```
...
Posso usare questo tipo di classificazione dei dati:

| Trasparency Class | class_model | Final purposes                            |
|-------------------|-------------|-------------------------------------------|
| 0 - 1             | H           | Training data for the "Healthy" Class     |
| 2 - 3 - 4         | S           | Training data for the "Symptomatic" Class |

Oppure posso usare questa classificazione:

| Trasparency Class | class_model |                                                |
|-------------------|-------------|------------------------------------------------|
| 0                 | Healthy     | trees that do not show any symptom of disease. |
| 1                 | Suspiciosus | trees that show low decline symptoms           |
| 2 - 3 - 4         | Symptomatic | trees in advanced state of decline             |

|                     |                      |
|---------------------|----------------------|
| **AREA**            | **Data valutazione** |
| **Pardulette**      | 05/09/2023           |
| **Santa Cristina**  | 05/09/2023           |
| **Pont'ezzu**       | 05/09/2023           |
| **Monte Scuccurau** | 07/09/2023           |
| **Tiriedu**         | 06/10/2023           |
| **Sos Cuzzos**      | 13/12/2023           |
| **Sos Cuzzos**      | 13/12/2023           |
| **Iscala Erveghe**  | 13/12/2023           |
| **Biarzu**          | **-**                |
| **Magrina e figu**  | \-                   |
...
# Assicurati che la colonna 'date' sia di tipo Date
vi_aggregated$date <- as.Date(vi_aggregated$date)

# Estrai le righe con data nel mese di agosto 2023
vi_aggregated_august <- subset(vi_aggregated, format(date, "%Y-%m") == "2023-08")
...
# Aggiungi la nuova colonna "CLASS_MODEL" in base ai criteri specificati
vi_aggregated_august <- vi_aggregated_august %>%
  mutate(CLASS_MODEL = case_when(
    CLASS %in% c(0) ~ "Healthy",
    CLASS %in% c(1) ~ "Suspiciosus",
    CLASS %in% c(2, 3, 4) ~ "Symptomatic",
    TRUE ~ NA_character_  # Se nessuna delle condizioni è soddisfatta, imposta a NA
  ))
...
```{r}
# Load the file of the class values and VI values
...
vi_aggregated_august$CLASS_MODEL <- factor(vi_aggregated_august$CLASS_MODEL)
...
class_counts <- table(train_data$CLASS_MODEL)
...
# Crea un indice di campionamento stratificato casuale
index <- createDataPartition(train_data$CLASS_MODEL, p = 0.8, list = FALSE, times = 1)

# Seleziona le righe corrispondenti agli indici stratificati
train_data_stratified <- train_data[index, ]

# Conta il numero di elementi per ogni classe nella colonna "CLASS_MODEL" nel nuovo set di addestramento
class_counts_stratified <- table(train_data_stratified$CLASS_MODEL)

# Stampa i risultati
print(class_counts_stratified)
```

```{r}
# Calcola gli indici per il test (complemento di train)
test <- setdiff(1:nrow(vi_aggregated_august), train_data_stratified)
...
rf_fit <- randomForest(CLASS_MODEL ~ . - CLASS - COD, 
                       data = train_data_stratified,
                       importance=TRUE,
                       proximity=TRUE)
...
######
MDSplot(rf_fit, vi_aggregated_august$CLASS_MODEL)
...
conf_matrix <- table(predictions, test_data$CLASS_MODEL)
print(conf_matrix)
...
```{r}
# Sostituisci "CLASS_MODEL" con il nome corretto della colonna target che vuoi prevedere
# Utilizziamo l'approccio positivo per selezionare le colonne da mantenere
model <- train(CLASS_MODEL ~ ., data = train_data[, colnames(train_data) %in% c("CLASS_MODEL", "BLUE", "DVI", "EVI", "GCI", "GDVI", "GEMI", "GNDVI", "GOSAVI", "GREEN", "GRVI", "GSAVI", "IPVI", "MSAVI2", "MSR", "NDVI", "NDWI", "NIR", "NLI", "OSAVI", "RDVI", "RED", "SAVI", "SR", "VARI")], method = "rf")

# Creiamo un nuovo set di dati per le previsioni
# Assicurati che il dataframe "vi_aggregated_august" contenga le stesse colonne presenti nel set di addestramento
data_for_model <- vi_aggregated_august[, colnames(train_data)]

# Fai previsioni sul nuovo set di dati
predictions <- predict(model, newdata = test_data)

# Valutazione del modello di classificazione
conf_matrix <- confusionMatrix(predictions, test_data$CLASS_MODEL)
